...
def scaled_dot_product_attention(q, k, v, mask, attention_mask=None):
    # calculate attention
    matmul_qk = paddle.matmul(q, k.transpose([0, 1, 3, 2]))
    # 6
    save_data(matmul_qk)
    scaled_attention_logits = matmul_qk / paddle.to_tensor(np.sqrt(k.shape[-1]),dtype=k.dtype)
    # 7
    save_data(scaled_attention_logits)

    # 这里是决定是否加上mask
    if mask is not None:
        nd, ns = scaled_attention_logits.shape[-2], scaled_attention_logits.shape[-1]
        scaled_attention_logits = scaled_attention_logits + mask[ns - nd : ns, :ns] * -1e4
        # 8
        save_data(mask)
        # 9
        save_data(mask[ns - nd : ns, :ns] * -1e4)
        # 10
        save_data(scaled_attention_logits)
...
class CTRLModel(CTRLPreTrainedModel):
    def __init__(self, **kwargs):
        ...
        self.pos_encoding = SinusoidalPositionalEmbedding(
            pd_config.n_positions, self.d_model_size
        )
    ...
    def forward(
        ...
        if token_type_ids is not None:
            ...
            token_type_embeds = self.w(token_type_ids) * paddle.to_tensor(np.sqrt(self.d_model_size),dtype=paddle.get_default_dtype())
        else:
            token_type_embeds = 0.0

        inputs_embeds = self.w(input_ids)
        
        inputs_embeds *= paddle.to_tensor(np.sqrt(self.d_model_size),dtype=paddle.get_default_dtype())
        pos_embeds = self.pos_encoding(position_ids)
        save_data(pos_embeds)
#         save_data(token_type_embeds)

        hidden_states = inputs_embeds + pos_embeds + token_type_embeds
    
        hidden_states = self.dropout(hidden_states)
        mask = paddle.triu(
            paddle.ones(shape=[seq_len + past_length, seq_len + past_length]), 1
        )
        # 0
...
class CTRLLMHeadModel(CTRLPreTrainedModel):
    def __init__(self, transformer):
        ...
        if self.pd_config.tie_word_embeddings:
            ...
            self.lm_head = nn.Linear(self.pd_config.n_embd, self.pd_config.vocab_size)
    ...
    def forward(
        ...
        if self.pd_config.tie_word_embeddings:
            lm_logits = (
                paddle.matmul(hidden_states, self.lm_head.weight, transpose_y=True)
                + self.lm_head_bias
            )
        else:
    ...
    def _reorder_cache(past, beam_idx):
        return tuple(
            tuple(past_state.index_select(beam_idx) for past_state in layer_past)
    ...
    def forward(
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(
                    pooled_logits.reshape([-1, self.num_labels]), labels.flatten()
                )
